"""Programmatic graders. Deterministic 0.0–0.98 scoring."""
...
class TaskGrader:
    ...
    def grade_episode(self, d: dict) -> float:
        ...
        return round(max(0.02, min(0.98, (r.score + 1) / 2)), 4)
    ...
    def _sev_score(self, d):
        ...
        if d.get("assessed_severity"):
            ...
            try:
                return max(0, 0.98 - abs(o.index(d["assessed_severity"]) - o.index(d["correct_severity"])) * 0.35)
            except ValueError:
                pass
        return 0.0001
    ...
    def _inv_score(self, d):
        ...
        s = (0.5 + cov * 0.3) if found else max(0.02, cov * 0.3)
    ...
    def _easy(self, d):
        ...
        return round(max(0.0201, min(0.98, t)), 4)
    ...
    def _hard(self, d):
        ...
        if d.get("done") and d.get("steps_taken", 0) > 0:
            eff = max(0.02, 0.98 - d["steps_taken"] / d.get("max_steps", 25))
        eff = max(0.02, eff - d.get("repeated_actions", 0) * 0.05)
