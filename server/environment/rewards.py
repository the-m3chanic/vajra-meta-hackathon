...
class RewardCalculator:
    ...
    def calculate_step_reward(self, action: Action, phase_before: IncidentPhase,
        ...
        if action.action_type == ActionType.ASSESS_SEVERITY:
            ...
            bd.investigation_quality = min(1.0, iv / 0.12)
            score += iv
            ...
            if agent_rem == correct_rem and targets_root:
                bd.remediation_appropriateness = 1.0
                score += 0.20
        ...
        bd.penalty = max(-1.0, round(bd.penalty, 4))
    ...
    def calculate_episode_reward(self, data: dict) -> Reward:
        ...
        if assessed:
            if assessed == correct_sev:
                ...
                try:
                    ...
                    bd.triage_accuracy = max(0, 1.0 - diff * 0.35)
                except ValueError:
                    bd.triage_accuracy = 0.0
        ...
        if found_root:
            bd.investigation_quality = min(1.0, 0.5 + inv_rel * 0.1)
        elif evidence:
        ...
        if root_cat == "bad_deployment" and "deployment" in ev_types:
            bd.investigation_quality = min(1.0, bd.investigation_quality + 0.2)
        ...
        if data.get("done") and steps > 0:
            bd.efficiency = max(0, 1.0 - steps / mx)
